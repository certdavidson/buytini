...
class StoreContext
{
    ...
    const MODULE      = 'translater_pro';
    const VERSION     = '1.0.0';
    const KEY_LICENSE = 'module_oc_kit_translater_pro_license_key';
    const KEY_CACHE   = 'module_oc_kit_translater_pro_license_cache';
    const KEY_TRIAL   = 'module_oc_kit_translater_pro_trial_start';
    ...
    const SECRET_KEY   = 'f3a91c7e8b542d06e4f7a2b3c9d1e5f8a6b0c4d7e2f1a9b3c8d5e0f7a4b1c6_k';
    const SECRET_CACHE = 'f3a91c7e8b542d06e4f7a2b3c9d1e5f8a6b0c4d7e2f1a9b3c8d5e0f7a4b1c6_c';
    const SECRET_TRIAL = 'f3a91c7e8b542d06e4f7a2b3c9d1e5f8a6b0c4d7e2f1a9b3c8d5e0f7a4b1c6_t';
    ...
    private function getTrialStart(): int
    {
        ...
        $this->db->query(
            "INSERT INTO `" . DB_PREFIX . "setting`
             ...
             VALUES (0, 'module_oc_kit_translater_pro', '" . self::KEY_TRIAL . "', '" . $payload . "', 0)"
        );
        return $now;
    }
    ...
    private function saveKey(string $key): void
    {
        ...
        $this->db->query(
            "DELETE FROM `" . DB_PREFIX . "setting`
             WHERE `code` = 'module_oc_kit_translater_pro' AND `key` = '" . self::KEY_LICENSE . "' AND `store_id` = 0"
        );
        $this->db->query(
            "INSERT INTO `" . DB_PREFIX . "setting` (`store_id`, `code`, `key`, `value`, `serialized`)
             VALUES (0, 'module_oc_kit_translater_pro', '" . self::KEY_LICENSE . "', '" . $escaped . "', 0)"
        );
    }
    ...
    private function saveCache(array $apiResponse): void
    {
        ...
        if (!empty($existing->row)) {
            ...
            $this->db->query(
                "INSERT INTO `" . DB_PREFIX . "setting`
                 (`store_id`, `code`, `key`, `value`, `serialized`)
                 VALUES (0, 'module_oc_kit_translater_pro', '" . self::KEY_CACHE . "', '" . $json . "', 0)"
            );
        }
    }
}
