...
class StoreContext
{
    ...
    const MODULE      = 'easycheckout';
    const VERSION     = '1.0.0';
    const KEY_LICENSE = 'module_oc_kit_easycheckout_license_key';
    const KEY_CACHE   = 'module_oc_kit_easycheckout_license_cache';
    const KEY_TRIAL   = 'module_oc_kit_easycheckout_trial_start';
    const CACHE_TTL   = 15552000; // 6 months
    const GRACE_TTL   = 604800;   // 7 days
    const TRIAL_TTL   = 0;        // 0 = trial disabled

    // ⚠️ Local-mode keeps validation offline (HMAC keygen). Set to false to use the
    // remote API endpoint instead. Keep true for offline distribution.
    const LOCAL_MODE = true;

    // Derived secrets — different per usage to prevent cross-context attacks.
    const SECRET_KEY   = 'e74d83f0db1b9a78e569cbc19d3f01052cca3eacc54f02a394bb4eb9ec49532c_k';
    const SECRET_CACHE = 'e74d83f0db1b9a78e569cbc19d3f01052cca3eacc54f02a394bb4eb9ec49532c_c';
    const SECRET_TRIAL = 'e74d83f0db1b9a78e569cbc19d3f01052cca3eacc54f02a394bb4eb9ec49532c_t';
    ...
    public function isActive(): bool
    {
        ...
        if (!$key) return $this->isTrialActive();
    ...
    private function getTrialStart(): int
    {
        ...
        $this->db->query(
            "INSERT INTO `" . DB_PREFIX . "setting`
             ...
             VALUES (0, 'module_oc_kit_easycheckout', '" . self::KEY_TRIAL . "', '" . $payload . "', 0)"
        );
        return $now;
    }
    ...
    private function saveKey(string $key): void
    {
        ...
        $this->db->query(
            "DELETE FROM `" . DB_PREFIX . "setting`
             WHERE `code` = 'module_oc_kit_easycheckout' AND `key` = '" . self::KEY_LICENSE . "' AND `store_id` = 0"
        );
        $this->db->query(
            "INSERT INTO `" . DB_PREFIX . "setting` (`store_id`, `code`, `key`, `value`, `serialized`)
             VALUES (0, 'module_oc_kit_easycheckout', '" . self::KEY_LICENSE . "', '" . $escaped . "', 0)"
        );
    }
    ...
    private function saveCache(array $apiResponse): void
    {
        ...
        $cache = [
            ...
            'checked_at'     => $now,
            'last_success'   => ($apiResponse['valid'] ?? false) ? $now : (int)($prev['last_success'] ?? 0),
        ];
        ...
        if (!empty($existing->row)) {
            ...
            $this->db->query(
                "INSERT INTO `" . DB_PREFIX . "setting`
                 (`store_id`, `code`, `key`, `value`, `serialized`)
                 VALUES (0, 'module_oc_kit_easycheckout', '" . self::KEY_CACHE . "', '" . $json . "', 0)"
            );
        }
    }
}
