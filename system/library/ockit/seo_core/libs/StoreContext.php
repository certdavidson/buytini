...
class StoreContext
{
    ...
    const MODULE      = 'seo_core';
    const VERSION     = '1.0.0';
    const KEY_LICENSE = 'module_oc_kit_seo_core_license_key';
    const KEY_CACHE   = 'module_oc_kit_seo_core_license_cache';
    const KEY_TRIAL   = 'module_oc_kit_seo_core_trial_start';
    ...
    const SECRET_KEY   = 'f7c19d82e4b3a051609fe2d7c845b39a2e87f1c6d0394a5b8e72061fd3c9a847_k';
    const SECRET_CACHE = 'f7c19d82e4b3a051609fe2d7c845b39a2e87f1c6d0394a5b8e72061fd3c9a847_c';
    const SECRET_TRIAL = 'f7c19d82e4b3a051609fe2d7c845b39a2e87f1c6d0394a5b8e72061fd3c9a847_t';
    ...
    private function getTrialStart(): int
    {
        ...
        $this->db->query(
            "INSERT INTO `" . DB_PREFIX . "setting`
             ...
             VALUES (0, 'module_oc_kit_seo_core', '" . self::KEY_TRIAL . "', '" . $payload . "', 0)"
        );
        return $now;
    }
    ...
    private function saveKey(string $key): void
    {
        ...
        $this->db->query(
            "DELETE FROM `" . DB_PREFIX . "setting`
             WHERE `code` = 'module_oc_kit_seo_core' AND `key` = '" . self::KEY_LICENSE . "' AND `store_id` = 0"
        );
        $this->db->query(
            "INSERT INTO `" . DB_PREFIX . "setting` (`store_id`, `code`, `key`, `value`, `serialized`)
             VALUES (0, 'module_oc_kit_seo_core', '" . self::KEY_LICENSE . "', '" . $escaped . "', 0)"
        );
    }
    ...
    private function saveCache(array $apiResponse): void
    {
        ...
        if (!empty($existing->row)) {
            ...
            $this->db->query(
                "INSERT INTO `" . DB_PREFIX . "setting`
                 (`store_id`, `code`, `key`, `value`, `serialized`)
                 VALUES (0, 'module_oc_kit_seo_core', '" . self::KEY_CACHE . "', '" . $json . "', 0)"
            );
        }
    }
}
