...
class GeneratorQueue
{
    public function __construct(
        ...
        private readonly RelatedWriter    $writer,
        private readonly array            $config  // module config array
    ) {}
    ...
    private function applyResultSort(array $topIds, array $candidates, string $sort): array
    {
        ...
        usort($topIds, function ($a, $b) use ($sort, $meta) {
            ...
            return match ($sort) {
                'price_asc'  => (float)($ra['price']      ?? 0) <=> (float)($rb['price']      ?? 0),
                'price_desc' => (float)($rb['price']      ?? 0) <=> (float)($ra['price']      ?? 0),
                'new'        => strcmp((string)($rb['date_added'] ?? ''), (string)($ra['date_added'] ?? '')),
                'name'       => strcmp((string)($ra['name']       ?? ''), (string)($rb['name']       ?? '')),
                default      => 0,
            };
        });
