...
class StoreContext
{
    ...
    const MODULE      = 'auto_related';
    const VERSION     = '1.0.0';
    const KEY_LICENSE = 'module_oc_kit_auto_related_license_key';
    const KEY_CACHE   = 'module_oc_kit_auto_related_license_cache';
    const KEY_TRIAL   = 'module_oc_kit_auto_related_trial_start';
    const CACHE_TTL   = 15552000; // 6 months
    const GRACE_TTL   = 604800;   // 7 days
    const TRIAL_TTL   = 604800;   // 7 days

    // ⚠️ DEV ONLY — set to false before release
    const LOCAL_MODE = true;

    // Derived secrets — different per usage to prevent cross-context attacks
    const SECRET_KEY   = '2a3d03478bc74aace11f4ab021e16fb59527aa66ff1e3bb668670afc165c14be_k';
    const SECRET_CACHE = '2a3d03478bc74aace11f4ab021e16fb59527aa66ff1e3bb668670afc165c14be_c';
    const SECRET_TRIAL = '2a3d03478bc74aace11f4ab021e16fb59527aa66ff1e3bb668670afc165c14be_t';
    ...
    {
        // Persist the key first (editSettingValue in ocStore only does UPDATE, not INSERT)
        $this->saveKey($key);
    ...
    private function getTrialStart(): int
    {
        ...
        if (!empty($result->row)) {
            ...
            if (is_array($stored) && isset($stored['ts'], $stored['sig'])) {
                ...
                if (!hash_equals($this->signTrial($ts), $stored['sig'])) {
                    return 0; // tampered — treat as expired
                }
                return $ts;
            }
            return 0; // legacy unsigned value — treat as expired
        }

        // First install — record trial start with signature
        $now     = time();
        ...
        $this->db->query(
            "INSERT INTO `" . DB_PREFIX . "setting`
             ...
             VALUES (0, 'module_oc_kit_auto_related', '" . self::KEY_TRIAL . "', '" . $payload . "', 0)"
        );
        return $now;
    }
    ...
    private function saveKey(string $key): void
    {
        ...
        $this->db->query(
            "DELETE FROM `" . DB_PREFIX . "setting`
             WHERE `code` = 'module_oc_kit_auto_related' AND `key` = '" . self::KEY_LICENSE . "' AND `store_id` = 0"
        );
        $this->db->query(
            "INSERT INTO `" . DB_PREFIX . "setting` (`store_id`, `code`, `key`, `value`, `serialized`)
             VALUES (0, 'module_oc_kit_auto_related', '" . self::KEY_LICENSE . "', '" . $escaped . "', 0)"
        );
    }
    ...
    private function saveCache(array $apiResponse): void
    {
        ...
        if (!empty($existing->row)) {
            ...
            $this->db->query(
                "INSERT INTO `" . DB_PREFIX . "setting`
                 (`store_id`, `code`, `key`, `value`, `serialized`)
                 VALUES (0, 'module_oc_kit_auto_related', '" . self::KEY_CACHE . "', '" . $json . "', 0)"
            );
        }
    }
}
